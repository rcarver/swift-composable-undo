...
public final class GameViewController: UIViewController {
  ...
  struct ViewState: Equatable {
    ...
    init(state: GameState) {
      ...
      self.canUndo = state.$boardState.canUndo
      self.canRedo = state.$boardState.canRedo
    }
  }
  ...
  public override func viewDidLoad() {
    ...
    self.navigationItem.rightBarButtonItems = [
      .init(barButtonSystemItem: .redo, target: self, action: #selector(redoButtonTapped)),
      .init(title: "│", style: .plain, target: nil, action: nil),
      .init(barButtonSystemItem: .undo, target: self, action: #selector(undoButtonTapped)),
    ]
    ...
    self.viewStore.publisher.canUndo
      .sink { [weak self] isEnabled in
        self?.navigationItem.rightBarButtonItems?[2].isEnabled = isEnabled
      }
      .store(in: &self.cancellables)

    self.viewStore.publisher.canRedo
      .sink { [weak self] isEnabled in
        self?.navigationItem.rightBarButtonItems?[0].isEnabled = isEnabled
      }
      .store(in: &self.cancellables)
  }
  ...
  @objc private func undoButtonTapped() {
    self.viewStore.send(.checkpoint(.undo))
  }

  @objc private func redoButtonTapped() {
    self.viewStore.send(.checkpoint(.redo))
  }
