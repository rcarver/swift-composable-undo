  ...
  public var body: some View {
    GeometryReader { proxy in
      WithViewStore(self.store.scope(state: ViewState.init)) { viewStore in
        ...
        .onAppear {
          viewStore.send(.checkpoint(.attachManager(undoManager)))
        }
        .navigationBarTitle("Tic-tac-toe")
        .navigationBarBackButtonHidden(true)
        .navigationBarItems(
          leading: Button("Quit") { viewStore.send(.quitButtonTapped) },
          trailing: HStack {
            Button("Undo") {
              viewStore.send(.checkpoint(.undo))
            }
            .disabled(!viewStore.canUndo)
            Divider()
            Button("Redo") {
              viewStore.send(.checkpoint(.redo))
            }
